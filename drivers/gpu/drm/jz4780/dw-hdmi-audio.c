...
struct snd_dwc_hdmi {
	struct device *dev;
	struct dwc_hdmi_audio_data data;
...
int dw_hdmi_jack_detect(struct snd_soc_codec *codec_dai,
			struct snd_soc_jack *jack)
{
	struct snd_dwc_hdmi *hdmi = snd_soc_codec_get_drvdata(codec_dai);
...
{
	struct snd_dwc_hdmi *hdmi = container_of(work, struct snd_dwc_hdmi,
						jack_work.work);
	u8 status;
...
static irqreturn_t snd_dwc_hdmi_irq(int irq, void *dev_id)
{
	struct snd_dwc_hdmi *hdmi = dev_id;
...
void enable_dw_hdmi_audio(struct snd_dwc_hdmi *hdmi)
{
...
{
	struct dwc_hdmi_audio_data *data = pdev->dev.platform_data;
	...
	ret = devm_request_irq(&pdev->dev, hdmi->data.irq, snd_dwc_hdmi_irq,
...
{
	struct snd_dwc_hdmi *hdmi = platform_get_drvdata(pdev);
